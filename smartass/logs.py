...
def how_to_configure_alias(configuration_details):
    print(u"Seems like {bold}damn{reset} alias isn't configured!".format(
        bold=color(colorama.Style.BRIGHT),
        reset=color(colorama.Style.RESET_ALL)))
    ...
    if configuration_details:
        ...
        if configuration_details.can_configure_automatically:
            print(
                u"Or run {bold}damn{reset} a second time to configure"
                u" it automatically.".format(
                    bold=color(colorama.Style.BRIGHT),
                    reset=color(colorama.Style.RESET_ALL)))

    print(u'More details - https://github.com/geoffyoon-dev/smartass-python#manual-installation')


def already_configured(configuration_details):
    print(
        u"Seems like {bold}damn{reset} alias already configured!\n"
...
def configured_successfully(configuration_details):
    print(
        u"{bold}damn{reset} alias configured successfully!\n"
        u"For applying changes run {bold}{reload}{reset}"
        u" or restart your shell.".format(
            bold=color(colorama.Style.BRIGHT),
            reset=color(colorama.Style.RESET_ALL),
...
def version(smartass_version, python_version, shell_info):
    sys.stderr.write(
        u'Smartass {} using Python {} and {}\n'.format(smartass_version,
                                                       python_version,
                                                       shell_info))
